...
contract PrivateSale is Ownable{
  ...
  constructor(uint256 _rate, address payable _wallet, BRANDON _token, uint256 endTime,address payable _holder) public payable {
    require(_rate > 0);
  ...
  function buyTokens(address _beneficiary) public payable {
    ...
    //DO HERE WHEN FIRST SALe OCCUR 
     saleUsers[_beneficiary] = SaleUser(block.timestamp,saleUsers[_beneficiary]._countOfSales += 1,saleUsers[_beneficiary]._totalAmountOfWeis +=valueAfterThreePercentTax); 
    
    }
    else if( block.timestamp < saleUsers[_beneficiary]._lastSaleTimestamp + 10 minutes &&  saleUsers[_beneficiary]._totalAmountOfWeis > fiftyFiveHundredInWei ){
        
        // Do sales tax here more then 5500 per week 
          saleUsers[_beneficiary] = SaleUser(block.timestamp,saleUsers[_beneficiary]._countOfSales += 1,saleUsers[_beneficiary]._totalAmountOfWeis +=msg.value);  
    }
    else if( block.timestamp < saleUsers[_beneficiary]._lastSaleTimestamp + 10 minutes){
    ...
    _forwardFunds();
    // _forwardFundsToHolder(valueAfterThreePercentTax);
  ...
  function _preValidatePurchase(
    ...
    view internal  hasClosed hasTokens
  {
  ...
  function _forwardFunds() internal {
   ...
   function _forwardFundsToHolder(uint256 _value) internal {
